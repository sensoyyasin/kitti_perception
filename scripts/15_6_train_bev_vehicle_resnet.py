...
class BasicBlock(nn.Module):
    ...
    Shortcut path:
        identity connection, or 1x1 Conv if shape changes
    """
...
class ResNet18UNet(nn.Module):
    """
    ResNet18-style encoder + U-Net decoder.

    Input:  B x 4 x H x W
    Output: B x 1 x H x W logits

    This is not ImageNet ResNet18 exactly.
    It is adapted for BEV segmentation.
    """
    def __init__(self, in_channels=4, out_channels=1):
        super().__init__()
...
def build_model(model_type):
    ...
    if model_type == "resnet18_unet":
        return ResNet18UNet(in_channels=4, out_channels=1)
